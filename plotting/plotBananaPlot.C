...
int plotBananaPlot(
 ...
 TChain fscChain("fscanalyzer/fscdigi");
  ...
  TTreeReaderValue<float> hFEsumPlus(eventReader, "hiHFplus"); 
  TTreeReaderValue<float> hFEsumMinus(eventReader, "hiHFminus");
  ...
  for (Long64_t i = 0; i < totalEvents; i++) {
    ...
    // coindidence filter selection 
    if(*coincidence != 1) continue; 

    // minimum bias selection 
    if(*minBias != 1) continue; 
    ...
    TFile* outfile = new TFile("BananaPlot_FSCHadronic.root",  "RECREATE");
    outfile->cd();
    bananaPlot->Write(); 
